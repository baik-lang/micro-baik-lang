  ...
  do
  {
    ...
    if (lex.type == _LONCATLABEL)
    {
      ...
      ValLabel(lex.detail.ident, sub_deep, valdat, VAL_FLAG_SEARCH_W);
        ...
        if (lex.type == _KODE_LUAR)
    {
      isIncluded = 1;
      // printf("INCLUDE KODE_LUAR ...\n");
...
/*  === WEBSTYLE Handle     ================================= end ==== */
  ...
  case TYPE_IDENT:
    ...
    else if (lex.detail.symbol == '+' && lex.type == TYPE_SYM)
    { // plusplus
      if (plusplus == 1)
      {
        // printf("plusplus \n");
        ...
        else if (tmpdat.ident != NULL && tmpdat.datatype == 1)
        {
